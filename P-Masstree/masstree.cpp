...
// for the last 1 byte is at the end of the second uint64_t.
    ...
    leafnode::leafnode(void *left, uint64_t key, void *right, uint32_t level = 1) : permutation(
            permuter::make_empty()) {
        level_ = level;
        next.store(NULL, std::memory_order_release);
    ...
    void leafnode::check_for_recovery(masstree *t, leafnode *left, leafnode *right, void *root, uint32_t depth,
                                      leafvalue *lv) {
...
//        entry[p].reference--;
    ...
    leafnode *leafnode::search_for_leftsibling(std::atomic<void *> *aroot, void **root, uint64_t key, uint32_t level,
                                               leafnode *right) {
        leafnode *p = NULL;
        key_indexed_position kx_;
        leafnode *next = NULL;
    ...
    void *leafnode::leaf_insert(masstree *t, void *root, uint32_t depth, leafvalue *lv, uint64_t key, void *value,
                                key_indexed_position &kx_) {
    ...
    void *leafnode::leaf_delete(masstree *t, void *root, uint32_t depth, leafvalue *lv, key_indexed_position &kx_,
    ...
    void *leafnode::inter_insert(masstree *t, void *root, uint32_t depth, leafvalue *lv,
                                 uint64_t key, void *value, key_indexed_position &kx_, leafnode *child,
    ...
    int leafnode::inter_delete(masstree *t, void *root, uint32_t depth, leafvalue *lv, key_indexed_position &kx_,
                               ThreadInfo &threadInfo) {
        int ret, merge_state;
    ...
    int masstree::merge(void *left, void *root, uint32_t depth, leafvalue *lv, uint64_t key, uint32_t level,
                        ThreadInfo &threadInfo) {
        leafnode *p = NULL;
        key_indexed_position kx_;
        leafnode *next = NULL;
