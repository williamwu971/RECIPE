...
for k, v in times.items():
    for i in range(len(v)):
        ...
        v[i] = "{}ns ({:.2f}\%)".format(int(v[i] * 100), v[i] / sum_list[i] * 100)
        # v[i] *= 100
...
for k in order:
    ...
    for number in v:
        ...
        if base == 0.0:
            template = template.replace("PPP", "{}ns".format(int(number)), 1)
            base = number
        else:
            # template = template.replace("PPP", "{:.2f} ({:+d}\%)".format(number, int((number - base) / base * 100)), 1)
            template = template.replace("PPP", "{}ns ({:+d}\%)".format(int(number), int((number - base) / base * 100)), 1)
